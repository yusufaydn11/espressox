-- 5 damga dolunca QR taraması = ücretsiz kahve kullan + kart sıfırla (5 damga redeemed)
...
BEGIN
  ...
  -- 5+ damga varken stamp taraması → ödül kullan (5 damga kapat, kart sıfırla)
  IF p_action = 'stamp' AND v_active_stamps >= v_stamps_required THEN
    v_redeem_batch := v_stamps_required;
    ...
    RETURN jsonb_build_object(
      'error', null,
      'redeemed', true,
      'stamps_redeemed', v_redeem_batch,
      'points_awarded', 0,
      'customer_id', v_customer,
      'remaining_stamps', GREATEST(0, v_active_stamps - v_redeem_batch)
    );
  END IF;
  ...
  SELECT count(*) INTO v_active_stamps
  FROM loyalty_stamps WHERE user_id = v_customer AND redeemed = false;
