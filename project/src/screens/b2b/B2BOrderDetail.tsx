...
import { ArrowLeft, Truck, Package, Calendar, CreditCard, CheckCircle2, ExternalLink, Banknote, Building2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import {
  orderService, paymentService, invoiceService,
  b2bFormatTRY, b2bFormatDate, b2bFormatDateTime,
  B2B_ORDER_STATUS_LABELS, B2B_ORDER_STATUS_TONES,
...
const PAYMENT_METHODS = [
  { id: 'bank_transfer', label: 'Havale / EFT', icon: Building2, desc: 'Banka transferi ile öde' },
  { id: 'card', label: 'Kredi Kartı', icon: CreditCard, desc: 'İyzico ile öde (yakında)' },
];
...
export function B2BOrderDetail({ orderId, onBack, showToast }: { orderId: string; onBack: () => void; showToast: ToastFn }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await orderService.getWithItems(orderId);
      setOrder(data);
      if (data && data.status !== 'awaiting_payment' && data.status !== 'cancelled') {
        const { data: inv } = await (await import('@/lib/supabase')).supabase
          ...
          .eq('order_id', orderId)
          .maybeSingle();
        setInvoice(inv as B2BInvoice | null);
      }
    } catch (e) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      showToast('Ödeme alındı, sipariş işleme alındı');
  ...
  const openInvoicePdf = () => {
    if (!invoice) return;
    const url = invoiceService.getInvoicePdfUrl(invoice.id);
    Linking.openURL(url);
  };

  if (loading) return <B2BScreenWrapper><B2BLoadingSpinner label="Sipariş yükleniyor…" /></B2BScreenWrapper>;
  if (error) return <B2BScreenWrapper><B2BErrorState message={error} onRetry={load} /></B2BScreenWrapper>;
  if (!order) return <B2BScreenWrapper><B2BErrorState message="Sipariş bulunamadı" /></B2BScreenWrapper>;
  ...
  return (
    <B2BScreenWrapper>
      ...
      <View className="rounded-2xl bg-white border border-ink-100 shadow-card p-5 mb-4">
        ...
        <View>
          {order.b2b_order_items.map((item, i) => (
            <View key={item.id} className={cn('flex-row items-center justify-between py-2.5', i > 0 && 'border-t border-ink-50')}>
        ...
        <View className="border-t border-ink-100 mt-4 pt-4 gap-2">
          <View className="flex-row justify-between"><Text className="text-sm text-ink-400">Ara Toplam</Text><Text className="text-sm font-medium text-ink-900">{b2bFormatTRY(order.subtotal)}</Text></View>
          <View className="flex-row justify-between"><Text className="text-sm text-ink-400">KDV</Text><Text className="text-sm font-medium text-ink-900">{b2bFormatTRY(order.vat_total)}</Text></View>
          <View className="flex-row justify-between"><Text className="text-sm font-bold text-ink-900">Genel Toplam</Text><Text className="text-lg font-bold text-ex-red">{b2bFormatTRY(order.total)}</Text></View>
        </View>
      </View>

      {order.notes ? (
        <View className="rounded-2xl bg-white border border-ink-100 shadow-card p-5 mb-4">
          <Text className="text-sm font-bold text-ink-900 mb-2">Sipariş Notu</Text>
      ...
      {(order.carrier_company || order.tracking_number || order.estimated_delivery) && (
        <View className="rounded-2xl bg-white border border-ink-100 shadow-card p-5 mb-4">
          ...
          <View className="flex-row flex-wrap gap-4">
            {order.carrier_company && <InfoItem icon={<Truck size={16} color="#6E6E78" />} label="Kargo" value={order.carrier_company} />}
            {order.tracking_number && <InfoItem icon={<Package size={16} color="#6E6E78" />} label="Takip No" value={order.tracking_number} />}
            {order.estimated_delivery && <InfoItem icon={<Calendar size={16} color="#6E6E78" />} label="Tahmini Teslim" value={b2bFormatDate(order.estimated_delivery)} />}
          </View>
          {order.tracking_url ? (
      ...
      {invoice && (
        <View className="rounded-2xl bg-white border border-ink-100 shadow-card p-5 mb-4">
          <View className="flex-row items-center justify-between mb-3">
            ...
            <B2BStatusBadge label={invoice.status === 'paid' ? 'Ödendi' : 'Açık'} tone={invoice.status === 'paid' ? 'green' : 'amber'} />
          </View>
          <Text className="text-sm text-ink-500 mb-3">Fatura No: {invoice.invoice_number}</Text>
          <Pressable onPress={openInvoicePdf} className="flex-row items-center justify-center gap-2 py-2.5 rounded-xl bg-ink-50">
            <ExternalLink size={16} color="#3D3D42" />
            <Text className="text-sm font-semibold text-ink-600">Faturayı Görüntüle / PDF İndir</Text>
          </Pressable>
        </View>
      )}
      ...
      <View className="rounded-2xl bg-white border border-ink-100 shadow-card p-5">
          ...
          <>
            {!showPaymentSheet ? (
              <Pressable
                ...
                className="flex-row items-center justify-center gap-2 py-3 rounded-xl bg-ex-red active:bg-ex-redDark mb-3"
              >
              ...
              <View className="mb-3">
                <Text className="text-xs font-semibold text-ink-500 uppercase tracking-wide mb-3">Ödeme Yöntemi Seçin</Text>
                <View className="gap-2 mb-4">
                  {PAYMENT_METHODS.map(m => {
                    ...
                    return (
                      <Pressable
                        key={m.id}
                        onPress={() => !isCard && setSelectedMethod(m.id)}
                        disabled={isCard}
                        className={cn(
                          'flex-row items-center gap-3 p-3.5 rounded-xl border',
                          selected ? 'border-ex-red bg-ex-red/5' : 'border-ink-100 bg-white',
                          isCard && 'opacity-50',
                        )}
                      >
                        <View className={cn('h-9 w-9 rounded-lg items-center justify-center', selected ? 'bg-ex-red/10' : 'bg-ink-50')}>
                          <Icon size={18} color={selected ? '#C8102E' : '#6E6E78'} />
                        </View>
                        <View className="flex-1">
                          <Text className="text-sm font-semibold text-ink-900">{m.label}</Text>
                          <Text className="text-[11px] text-ink-400">{isCard ? 'Yakında kullanıma açılacak' : m.desc}</Text>
                        </View>
                ...
                <View className="flex-row gap-2">
                  <Pressable
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-ink-50 items-center"
                  >
                  ...
                  <Pressable
                    ...
                    disabled={paying}
                    className="flex-[2] flex-row items-center justify-center gap-2 py-2.5 rounded-xl bg-ex-red disabled:opacity-40"
                  >
                    {paying ? (
                      ...
                      <>
                        ...
                        <Text className="text-sm font-semibold text-white">{b2bFormatTRY(order.total)} Öde</Text>
                      </>
                    )}
                  </Pressable>
                </View>
        ...
        {order.status === 'paid' && (
          <View className="flex-row items-center gap-2"><CheckCircle2 size={18} color="#16a34a" /><Text className="text-sm text-green-600">Ödeme alındı, merkez onayı bekleniyor</Text></View>
        )}
